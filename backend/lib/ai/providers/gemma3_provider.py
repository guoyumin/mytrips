"""
Gemma3 Provider - Local Gemma3 models via Ollama
"""
import json
import os
import logging
...
class Gemma3Provider(AIProviderInterface):
    """Gemma3 implementation of AI Provider Interface for local models via Ollama"""
    
    def __init__(self, model_version: str = None):
        # Load config first to get default model if not specified
        self.config = self._load_config()
        self.base_url = self.config.get('base_url', 'http://localhost:11434')
        self.model_version = model_version or self.config.get('model', 'gemma3:12b')
        
        # Test connection to Ollama server
        try:
            self._test_connection()
            logger.info(f"Initialized Gemma3 provider: {self.model_version} at {self.base_url}")
        except Exception as e:
            logger.error(f"Failed to initialize Gemma3 provider: {e}")
            raise
    ...
    def _load_config(self) -> Dict:
        ...
        config_path = os.path.join(project_root, 'config', 'gemma3_config.json')
        ...
        if not os.path.exists(config_path):
            logger.warning(f"Gemma3 config not found at {config_path}, using defaults")
        ...
        try:
            ...
            logger.error(f"Error loading Gemma3 config: {e}, using defaults")
            return default_config
    
    def get_model_info(self) -> Dict:
        """Get information about the Gemma3 model"""
        model_size = "12B" if "12b" in self.model_version.lower() else "27B" if "27b" in self.model_version.lower() else "Unknown"
        return {
            "provider": "Gemma3 (Local via Ollama)",
            "model_name": f"Gemma 3 {model_size}",
